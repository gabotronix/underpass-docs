...
For instance, if you create a `Server` or a `User` in the Pritunl web panel, the data is stored in the `underpass_mongodb` volume. Even if you remove or delete the `pritunl container`, your login credentials, users, and VPN servers will remain intact. However, if you delete the `underpass_mongodb` volume, all your data is lost as well.
